...
export const UnlockRoom = ({ room }) => {
  ...
  const [unlockRoom, { isLoading }] = useLockRoomMutation();

  const handleFormSubmit = async (e) => {
    e.preventDefault();
    try {
      const unlockRoomRequest = {
        isLocked: false as false,
      };

      const response = await unlockRoom({
        body: unlockRoomRequest,
        id: room.id || "",
      });
  ...
  return (
    <>
      <button
        ...
        onClick={openModal}
      >
        <FaLockOpen  className="flex-shrink-0 size-3.5" />
        Unlock
      </button>
              ...
              <Dialog.Panel className="relative inline-flex w-full transform flex-col overflow-hidden rounded-xl bg-white dark:bg-slate-800 text-left align-bottom shadow-2xl transition-all sm:my-8 md:my-32 sm:max-w-md sm:align-middle">
                ...
                <div className="flex-1 px-6 py-5 sm:py-6">
                  ...
                  <div className="flex-1  py-5 sm:py-6 dark:text-white">
                    <p>Are you sure you want to unlock this room?</p>
                  </div>
                </div>
